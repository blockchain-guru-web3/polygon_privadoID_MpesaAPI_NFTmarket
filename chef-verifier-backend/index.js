...
// @ts-ignore
app.get("/api/proveChef", async (req, res) => {
	console.log("Verify chef query request received");
...
async function getQueryRequest(req, res) {
	...
	const proofRequest = {
		...
		query: {
			allowedIssuers: ["*"],
			context: "https://mraztevin.github.io/chef-verifier.github.io/",
			type: "chef verification",
			credentialSubject: {
				"Code": {
					"$in": [
						39,
